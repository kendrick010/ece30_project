...
EXHAUSTIVE_CHECK:
	LDA X0, a
	ADDI X19, XZR, #1							// Max length of array for i, j < length(ARRAY)
	MOV X1, XZR									// i = 0
	MOV X2, XZR									// j = 0
	...
	LOOP_OUTER:
		MOV XZR, XZR							// dummy command

		CHECK_INNER:
			CMP X2, X1							// Edge case: skip when j < i
			B.LT ITERATE
		...
		LOOP_INNER:
			...
			ADDI X20, XZR, #0	   				// Initializing a counter for restoring array
			BL RESTORE_ARRAY
			LDA X0, a
			...
			ADDI X20, XZR, #0	   				// Initializing a counter for printing
			BL PRINTLIST
